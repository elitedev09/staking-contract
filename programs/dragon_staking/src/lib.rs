...
declare_id!("9YsYjZjmxV71wgHAxtYrSoJ7gr4rbJeDmyG4rmj8Y5f");
...
pub mod dragon_staking {
    ...
    pub fn change_tokens_per_second(
        ...
        for pool_acc in _ctx.remaining_accounts.iter() {
            let loader = AccountLoader::<FarmPoolAccount>::try_from(&pool_acc)?;
    ...
    pub fn harvest(_ctx: Context<Harvest>) -> ProgramResult {
        ...
        let total_reward = user.reward_amount.checked_add(user.extra_reward).unwrap();
...
pub struct CreateFarmPool<'info> {
    ...
    pub pool: AccountLoader<'info, FarmPoolAccount>,
    #[account(mut, seeds = [b"state".as_ref()], bump = state.load()?.bump, has_one = authority)]
    pub state: AccountLoader<'info, StateAccount>,
...
pub struct CreatePoolUser<'info> {
    ...
    pub user: AccountLoader<'info, FarmPoolUserAccount>,
    #[account(seeds = [b"state".as_ref()], bump = state.load()?.bump)]
    pub state: AccountLoader<'info, StateAccount>,
...
pub struct CreateUserEtherAddress<'info> {
    ...
    pub user: AccountLoader<'info, FarmUserEtherAddress>,
    pub authority: Signer<'info>,
    pub system_program: UncheckedAccount<'info>,
}
...
pub struct FarmPoolUserAccount {
    ...
    pub extra_reward: u64, // extra from lock duration; ex lock 12M => +10%
    pub reward_debt: u64,
...
impl FarmPoolUserAccount {
    fn calculate_reward_amount<'info>(
        ...
        let pending_amount: u64 = u128::from(self.amount)
        ...
        let extra_amount: u64 = u128::from(pending_amount)
